...
```
push → build (CC_PRE_BUILD_HOOK) → post-build → run (CC_PRE_RUN_HOOK) → start → [health check]
```
...
```bash
# Snapshot (pipe to tail — clever logs streams continuously)
...
2. **Check logs**: `clever logs --app <id> --since 2h | tail -80`
